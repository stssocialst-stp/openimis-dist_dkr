...
if [[ -f '.init.lock' ]]; then
    ...
    # 3.3️⃣ Criar usuário do PostgreSQL se não existir
    docker compose exec db psql -U postgres -tc "SELECT 1 FROM pg_roles WHERE rolname='${DB_USER}'" | grep -q 1 || \
    ...
    psql -h db -U ${DB_USER} -tc \"SELECT 1 FROM pg_database WHERE datname='${DB_NAME}'\" | grep -q 1 || \
    createdb -h db -U ${DB_USER} ${DB_NAME}
    "
    docker compose exec db psql -U postgres -c "ALTER DATABASE ${DB_NAME} OWNER TO ${DB_USER};"
